...
test("ficha de producto accesible y confirmación dentro del diálogo", async ({ page }) => {
  await openCatalog(page);
...
test("lista, cantidades, nota, persistencia y mensaje de WhatsApp", async ({ page }) => {
  await openCatalog(page);
  ...
  await expect(dialog.getByText("2 líneas · 3 unidades")).toBeVisible();
  ...
  expect(message).toContain("1. Grasa de res — cantidad 2");
  expect(message).toContain("2. Ropa americana — cantidad 1");
  expect(message).toContain("Nota: Confirmar detalles, por favor.");
  expect(message).toContain("¿Me confirma disponibilidad, precio y forma de entrega?");

  await page.keyboard.press("Escape");
  await page.reload({ waitUntil: "networkidle" });
  await expect(page.locator("[data-consult-count]").first()).toHaveText("3");
  await page.locator("[data-consult-open]:visible").first().click();
...
test("recupera de almacenamiento local corrupto", async ({ page }) => {
  await page.goto("catalogo.html", { waitUntil: "networkidle" });
  await page.evaluate(() => {
    globalThis.localStorage.setItem("vendedor-scz:consultation:v1", "{dato roto");
  });
  await page.reload({ waitUntil: "networkidle" });
  await expect(page.locator("[data-consult-count]").first()).toHaveText("0");
  await page.locator("[data-consult-open]:visible").first().click();
...
test("alcanzar 999 unidades no provoca un error de página", async ({ page }) => {
  const pageErrors = [];
  page.on("pageerror", (error) => pageErrors.push(error.message));
  await page.goto("catalogo.html", { waitUntil: "networkidle" });
  await page.evaluate(() => {
    globalThis.localStorage.setItem("vendedor-scz:consultation:v1", JSON.stringify({
      schemaVersion: 1,
      items: [{ id: "grasa-de-res", name: "Grasa de res", quantity: 999 }],
      note: ""
    }));
  });
  await page.reload({ waitUntil: "networkidle" });
  ...
  await expect(page.locator("[data-consult-count]").first()).toHaveText("999");
